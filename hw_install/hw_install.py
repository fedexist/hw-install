...
def setup(_current_host, _username, ambari_server, _etc_host, is_ambari_server
	...
	try:
		...
		ssh_session.sendline("echo \"fs.file-max = 10000000\" | cat - >> /etc/sysctl.conf")
		ssh_session.prompt()
		ssh_session.sendline("echo \"* soft nofile 10000000\" | cat - >> /etc/security/limits.conf")
		ssh_session.prompt()
		ssh_session.sendline("echo \"* hard nofile 10000000\" | cat - >> /etc/security/limits.conf")
		ssh_session.prompt()
		ssh_session.sendline("echo \"* soft nproc 10000000\" | cat - >> /etc/security/limits.conf")
		ssh_session.prompt()
		ssh_session.sendline("echo \"* hard nproc 10000000\" | cat - >> /etc/security/limits.conf")
		ssh_session.prompt()
