    ...
    try:
        if not is_ambari_server:
            ...
            process = subprocess.Popen(
                "scp -q -o StrictHostKeyChecking=no /%s/.ssh/id_rsa /%s/.ssh/id_rsa.pub %s@%s:/%s/.ssh/"
...
def setup(_current_host, _username, ambari_repo, ambari_server, _etc_host, is_ambari_server):
    try:
        ...
        ssh_session.sendline("echo \"fs.file-max = 800000\" | cat - >> /etc/sysctl.conf")
        ssh_session.prompt()
        ssh_session.sendline("echo \"* soft nofile 800000\" | cat - >> /etc/security/limits.conf")
        ssh_session.prompt()
        ssh_session.sendline("echo \"* hard nofile 800000\" | cat - >> /etc/security/limits.conf")
        ssh_session.prompt()
        ssh_session.sendline("echo \"* soft nproc 800000\" | cat - >> /etc/security/limits.conf")
        ssh_session.prompt()
        ssh_session.sendline("echo \"* hard nproc 800000\" | cat - >> /etc/security/limits.conf")
        ssh_session.prompt()
